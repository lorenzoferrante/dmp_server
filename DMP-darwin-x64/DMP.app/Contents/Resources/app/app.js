...
var dataFile = path.join(__dirname, 'data.csv')
var dbDMPFile = path.join(__dirname, 'dmp_prod.csv')
  ...
  fs.createReadStream(file)
    ...
    var date_to_display = moment().format('L')
      ...
      var data_to_append = idx+';'+cod+';'+quant+';'+ubi+';'+des+';'+op_cod+';'+date_to_file+';'+date_to_display+';'+exe+'\n'
        ...
        fs.appendFile(dataFile, data_to_append, function (err) {
            if (err) throw err
...
io.on('connection', function (socket) {    
  ...
  socket.on('new product', function (cod, quant, op) { 
    if (op_cod == '') {
        ...
        // Cerco il prodotto 
        console.log('[DEBUG] after searchDB:', searchDB(dbDMPFile, cod, quant, op, idx, exe)) 
    }
  })
  
  socket.on('new checked', function (id) {
      console.log('Remove prod.with id:', id)
      utils.changeEXE(dataFile, id, true)
      //loadFromFile('data.csv')
  })
  
  socket.on('new search', function (data) {
      searchForDesc(dbDMPFile, data)
  })
